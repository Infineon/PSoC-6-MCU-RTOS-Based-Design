...
#define GetRed(x)       (uint8_t)((x)        & 0x000000FFu)
#define GetGreen(x)     (uint8_t)((x >> 8u)  & 0x000000FFu)
#define GetBlue(x)      (uint8_t)((x >> 16u) & 0x000000FFu)
...
{ 
    ...
    for(;;)
    {
        ...
        if(rtosApiResult == pdTRUE)
        {
            ...
            if ((GetIntensity(colorAndIntensity) < LED_NO_COLOR_THRESHOLD)||
               ((GetBlue(colorAndIntensity)      < LED_NO_COLOR_THRESHOLD)&&
                (GetGreen(colorAndIntensity)     < LED_NO_COLOR_THRESHOLD)&&
                (GetRed(colorAndIntensity)       < LED_NO_COLOR_THRESHOLD)))
            {           
                PWM_Red_Disable();
                PWM_Green_Disable();
                PWM_Blue_Disable();
            ...
            else
            {
                ...
                if (((Cy_TCPWM_PWM_GetStatus(PWM_Red_TCPWM__HW,
                        PWM_Red_TCPWM__CNT_IDX))
                ...
                Cy_TCPWM_Counter_SetCompare0(PWM_Red_TCPWM__HW, 
                                             PWM_Red_TCPWM__CNT_IDX, 
                                             ((GetRed(colorAndIntensity) * 
                                             GetIntensity(colorAndIntensity)) 
                                             >> COLOR_COMPONENT_SCALER_RG));
                Cy_TCPWM_Counter_SetCompare0(PWM_Green_TCPWM__HW, 
                                             PWM_Green_TCPWM__CNT_IDX,
                                             ((GetGreen(colorAndIntensity) *
                                             GetIntensity(colorAndIntensity))
                                             >> COLOR_COMPONENT_SCALER_RG));
                Cy_TCPWM_Counter_SetCompare0(PWM_Blue_TCPWM__HW, 
                                             PWM_Blue_TCPWM__CNT_IDX,
                                             ((GetBlue(colorAndIntensity) * 
                                             GetIntensity(colorAndIntensity)) 
